...
tf.logging.set_verbosity(tf.logging.WARN) # record warnning message.This can make it easier to monitor the progress of the training process and identify any issues that arise.

fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D, flip_input=False) 
'''
used for facial landmark detection
face_alignment.LandmarksType._2D: Specifies the type of facial landmarks to detect.
flip_input=False: Specifies whether to flip the input images horizontally before detecting facial landmarks.In this case, it specifies to detect 2D facial landmarks.
'''

EYE_H, EYE_W, NOSE_H, NOSE_W , MOUTH_H, MOUTH_W= 40, 40, 32, 40, 32, 48 #These are constants that define the height and width of the different facial regions in pixels.

class TPGAN():
'''
The TPGAN class is the main class for the TPGAN model, which contains the generator, discriminator, classifier, 
and other components necessary for training and generating synthetic temporal patterns.
'''
    ...
    '''
    The current_epoch_from_weights_file method of the TPGAN class extracts the number of epochs that have been trained and saved in a given weight file. 
    It takes as input the path to the weight file (weights_file) and returns the number of epochs as an integer.
    The method first checks if weights_file is not None. If it is not None, the method attempts to extract the 
    epoch number from the filename using a regular expression. 
    Specifically, it tries to match the filename to the pattern .*epoch([0-9]+).*.hdf5, 
    which looks for the string "epoch" followed by one or more digits, surrounded by any other characters and ending with ".hdf5". 
    If there is a match, the method extracts the epoch number as a string using the groups() method 
    of the re.Match object returned by re.match(). It then converts the epoch number to an integer and returns it.
    If weights_file is None or there is no match for the pattern in the filename, the method returns 0.
    '''
    ...
    def discriminator(self):
        ...
        The main benefit of using a getter method is that it provides encapsulation of the object's data, 
        preventing external code from modifying the data directly and providing a consistent way of accessing that data.
 
	    The discriminator method of the TPGAN class is a getter method for the discriminator network, which is responsible for distinguishing between real and generated temporal patterns.                     	If the discriminator network has not been created yet, the method calls the build_discriminator method to create it, passing in the base_filters attribute as a hyperparameter. Then, 	it loads the weights from the specified discriminator_weights file (if it is not None) using the load_weights method of the discriminator network. Finally, it returns the 	discriminator network.

	    The loss_disc function is defined inside the discriminator method and serves as the loss function for the discriminator network. It uses binary cross-entropy loss with one-sided 	label smoothing to train the discriminator to distinguish between real and generated patterns. The y_true argument contains the true labels for the patterns (0 for generated, 1 for 	real), and y_pred contains the predicted labels. The function returns the binary cross-entropy loss with label smoothing applied to the true labels.
        """
        ...
        if self._discriminator is None:
            ...
            self._discriminator = load_model(self.discriminator_weights,custom_objects= {'CloudableModel':CloudableModel,'tf':tf,'multipie_gen':multipie_gen,'loss_disc':loss_disc})
    ...
    def generator(self):
	    ...
	    In object-oriented programming, a "getter" method is a method that retrieves the value of a private or protected instance variable of an object. It allows external code to access the 	value of an object's private or protected variables without directly accessing those variables.

	    In this specific code, generator is a getter method that returns the generator network instance of the TPGAN class. The method checks if _generator is None, and if it is, it 	initializes a new generator network using the build_generator method and loads the weights from a pre-trained model file using the load_model method from Keras. Finally, it returns 	the generator network instance. If _generator is not None, it just returns the previously created generator network instance.
    ...
    def classifier(self):
	    ...
	    This method defines a getter function for the classifier model of the TPGAN class. 

	    If the `_classifier` attribute of the TPGAN instance is `None`, it means that the classifier model has not been instantiated yet, so it builds the classifier by calling the 	`build_classifier()` method and then loads the weights of the pre-trained classifier model from the specified `classifier_weights` file using the `load_model()` method from Keras. 	Finally, it returns the `_classifier` attribute, which now stores the pre-trained classifier model with loaded weights.
	    """
    ...
    def parts_rotator(self):
	    ...
	    In the TPGAN class, parts_rotator is another getter method that returns the part rotator network instance for each facial part (left eye, right eye, nose, and mouth).

	    The method first checks if _parts_rotator is None, and if it is, it initializes a new part rotator network using the build_parts_rotator method. This method creates a convolutional 	neural network that rotates the input image around a facial landmark by an angle specified as an input to the network.

	    Once the part rotator network has been initialized, the method returns the instance of the network. If _parts_rotator is not None, it simply returns the previously created instance 	of the part rotator network.
	    """
    ...
    def _add_activation(self, X, func='relu'):
	    ...
	    The _add_activation method is a private method in the TPGAN class that is used to add an activation layer to a given input tensor X.
        The method takes two inputs: X, which is the input tensor, and func, which specifies the type of activation function to be applied to the input tensor.
        If func is None, the method simply returns the input tensor. If func is 'relu', the method applies a ReLU activation function to the input tensor using the Activation layer provided by Keras. If func is 'lrelu', the method applies a leaky ReLU activation function to the input tensor using the LeakyReLU layer provided by Keras.
        If an unknown activation function is provided, the method raises an exception.
        """
    ...
    def _res_block(self, X, kernel_size, batch_norm=False, activation=None, name=None):
	    ...
	    This is a private function `_res_block` that is used to add a residual block to the neural network. A residual block is a group of layers in a neural network that allows for thetraining of deeper models by helping to prevent the vanishing gradient problem that can occur when training very deep neural networks. The function takes as input `X`, the output 	from the previous layer, `kernel_size`, the size of the kernel used in the convolutional layers, `batch_norm`, a boolean value indicating whether batch normalization should be used 	in the convolutional layers, `activation`, the activation function to be used after each convolutional layer, and `name`, a string used to name the layers in the block. The function 	performs two convolutional operations, applies batch normalization if specified, applies the activation function, adds the output to the input using an element-wise addition 	operation, and returns the result.
	    """
    ...
    def build_generator(self, name="generator", base_filters=64):
        ...
        #The purpose of performing multiple resizing operations on the input image could be to generate multiple versions of the image at different resolutions, which can then be used for different purposes in a neural network.
        ...
        #Base filters refer to the number of filters used in the first convolutional layer of a neural network architecture. 
        #a kernel size of (7, 7)
        #The strides argument is set to (1, 1), which means that the convolutional filter moves one pixel at a time in both the horizontal and vertical directions.
        #The padding argument is set to 'same', which means that the output tensor has the same spatial dimensions as the input tensor.
        #residual blocks allow memory (or information) to flow from initial to last layers.
        #if we keep increasing the number of layers, we will see that the accuracy will saturate at one point and eventually degrade. And, this is usually not caused due to overfitting.
        #So, it might seem that the shallower networks are learning better than their deeper counterparts, and this is quite counter-intuitive.
        #kernel_initializer and bias_initializer are used to initialize the weights and biases of the Conv2D layer.
        #kernel_initializer initializes the weights of the convolutional filter with a truncated normal distribution. The standard deviation of this distribution is set to 0.02 in this case.
        #bias_initializer initializes the bias vector with zeros.
        ...
        # This helps to refine the features in the tensor before passing it to the next layer.
        # size16
        ...
        #This function is used to resize an image tensor to a new height and width using bilinear interpolation. In this specific case,
        #the interpolated128 variable is created by resizing the img64 tensor to a new size of [128, 128] using bilinear interpolation.
        ...
        # Based on the names of the variables, it seems that this method is responsible for rotating the input images of the eyes, nose, 
        # and mouth to different angles or positions to create variations in the dataset. The rotated images are then split into parts 
        # (front_leye_img, front_reye_img, front_nose_img, front_mouth_img) and their corresponding features (front_leye_feat, front_reye_feat, front_nose_feat, front_mouth_feat).
        #  These rotated and split images and features are used as inputs to the generator network.
    ...
    def build_classifier(self, name='classifier'):
        ...
        In a two-pathway GAN, the classifier is used to enforce a constraint on the generator by ensuring that the generated images belong to the same class as the input images. 
        This is typically done by training a separate classifier on the same dataset as the generator and discriminator, 
        and then using the output of the classifier as an additional input to the generator.
        """
    ...
    def build_train_generator_model(self):
        ...
        img128_sym = Lambda(lambda x:x, name = "Triple_Consistency128_sym")(img128)
        ...
        subject_id = Lambda(lambda x:x, name = "contenet_classify")(subject_id)
    ...
    def build_discriminator(self, name='discriminator', base_filters=64):
        ...
        ret_model = CloudableModel(inputs=in_img, outputs=c4r, name=full_name)
        #ret_model.summary()
    ...
    def train_gan(self, gen_datagen_creator, gen_train_batch_size, gen_valid_batch_size,
                  ...
                  #lambda_class=4e-1, lambda_parts=3): changed 
    ...
    def train_generator(self, train_gen, valid_gen, optimizer=Adam(lr=0.0001), steps_per_epoch=100, epochs=1, is_output_img=False, out_dir='D:/desktop/tpgan_keras/keras_tpgan/out/', out_period=1,
        ...
        def Triple_Consistency128_sym(y_true, y_pred):
            """
            symmetricity loss for size 128x128
            """
            #loss_sym = tf.reduce_mean (np.abs(y_pred[:,:,::-1,:] - y_pred))
        ...
        def contenet_classify(y_true, y_pred):
            """
            classification loss
            """
            return lambda_class * tf.reduce_sum(losses.categorical_crossentropy(y_true, y_pred))
        ...
        if self.generator_train_model is None:
            ...
            self.generator_train_model.compile(optimizer=optimizer,
                                loss={'00img128px': _loss_img128_px,  
                                      'Triple_Consistency128_sym': Triple_Consistency128_sym,   # 01img128sym
                                      '02ip': _loss_ip,
                                      '03adv': _loss_adv,
                                      '04tv': _loss_tv,
                                      ...
                                      'contenet_classify':contenet_classify,      # 09classify
                                      '10leye': _loss_part,
                                      '11reye': _loss_part,
                                      '12nose': _loss_part,
                                      '13mouth': _loss_part},
    ...
    def generate(self, inputs):
        ...
        img64_ = (img64*np.iinfo(np.uint8).max).astype(np.uint8)
        img64 = np.array(255*(img64_)**0.5, dtype = 'uint8')
        img32_ = (img32*np.iinfo(np.uint8).max).astype(np.uint8)
        img32 = np.array(255*(img32_)**0.5, dtype = 'uint8')
