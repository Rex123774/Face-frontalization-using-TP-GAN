...
class Datagen():
    ...
    def __init__(self, dataset_dir='', landmarks_dict_file='',
        ...
        self.datalist: a list of paths to the input images used in training.
# valid_count: the number of images used for validation (i.e., not for training).
# self.train_list: a list of paths to the input images used for training (excluding the images used for validation).
# self.train_cursor: an integer indicating the current position in the list of training images.
# self.valid_list: a list of paths to the input images used for validation.
# self.valid_cursor: an integer indicating the current position in the list of validation images.
# self.mirror_to_one_side: a boolean indicating whether to mirror the input images horizontally.
# self.face_trembling_range: a float indicating the range of random perturbation applied to the input images.
# self.workers: an integer indicating the number of threads used to load and process the input images.
# self.thread_pool_executor: a thread pool executor object used to run the image processing in parallel.
# self.lock: a lock object used to synchronize access to the training and validation lists when used in a multithreaded environment.
    ...
    def __del__(self):
        if self.workers > 1:
            try:
                ...
                pass
# This is a destructor method in Python, which is automatically called when an object is deleted or goes out of scope.
# In this case, it is defined in the MultipieGen class and is responsible for shutting down the ThreadPoolExecutor instance that was created during the initialization of the class.       
                
    def batch_data(self, datalist, cursor, batch_size = 16):

        # This function is useful in the context of training a machine learning model, 
        # as it allows the model to receive a new batch of data for each training iteration, without having to load the entire dataset into memory at once.
        ret_list = []
